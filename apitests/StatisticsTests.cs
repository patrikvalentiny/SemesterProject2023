...
public class StatisticsTests
{
    ...
    [TestCase]
    [TestCase(90, 80, 100)]
    public async Task CalculatedWeeklyLossTest(decimal startWeight = 100, decimal targetWeight = 80, int dateDiff = 10)
    {
        await using var conn = Helper.OpenConnection();
        var weight = new WeightInput
        {
        ...
        try
        {
            response = await _httpClient.GetAsync(Url + "/calculatedWeeklyLoss");
            TestContext.WriteLine("THE FULL BODY RESPONSE: " + await response.Content.ReadAsStringAsync());
        }
        catch (Exception e)
        {
        ...
        using (new AssertionScope())
        {
            var expected = (weight.Weight - targetWeight) / dateDiff * 7;
            response.IsSuccessStatusCode.Should().BeTrue();
            responseObject.Should().Be(decimal.Round(expected, 2));
        }
    }
